...
- **No List view (decided 2026-09-20).** The owner dropped it, so the road is the only timeline and `Timeline.tsx` is gone. The fallback for keyboard and screen-reader use is the road itself: every marker is a real button in chronological order, the panel lists a year's experiences as buttons, and camera moves jump instead of animating under `prefers-reduced-motion`. Screen-reader use has not been tested with a real reader yet.
